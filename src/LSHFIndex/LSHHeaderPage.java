...
//   }
   ...
   private void unpinThisPage(boolean dirty) throws HashEntryNotFoundException, InvalidFrameNumberException, PageUnpinnedException, ReplacerException, IOException {
      SystemDefs.JavabaseBM.unpinPage(this.getCurPage(),dirty);
   }

   public void getHashFunctions(String indexName) throws InvalidSlotNumberException, IOException, FieldNumberOutOfBoundException, BufferPoolExceededException, PageNotReadException, HashOperationException, BufMgrException, PagePinnedException, InvalidFrameNumberException, PageUnpinnedException, ReplacerException, HashEntryNotFoundException, InvalidTupleSizeException, InvalidTypeException {
//      Tuple t = getRecord(new RID(this.headerPageId,0));
      if (this.getPageType() == HASH_FUNCTION_PAGE) {
         ...
         LSHashFunctionsMap.addHashFunction(lsHashFunction, indexName);
      }
      ...
      if (nextHeaderPage != null){
         nextHeaderPage.getHashFunctions(indexName);
      }
      return;
   }

   public void getLayers(int h, String indexName) throws BufferPoolExceededException, PageNotReadException, HashOperationException, BufMgrException, PagePinnedException, InvalidFrameNumberException, IOException, PageUnpinnedException, ReplacerException, FieldNumberOutOfBoundException, InvalidSlotNumberException, HashEntryNotFoundException, InvalidTupleSizeException, InvalidTypeException {
      if (this.getPageType() == LAYER_PAGE) {
         ...
         LSHLayer l = new LSHLayer(hashFunctions,layerSt.getIntFld(1),layerSt.getIntFld(2), indexName);
         LSHLayerMap.addLayer(l, indexName);
      }
      ...
      if (nextHeaderPage != null){
         nextHeaderPage.getLayers(h,indexName);
      }
      return;
   }
...
//      SystemDefs.JavabaseBM.unpinPage(this.headerPageId, true);
   ...
   public static class LSHHeaderPageFactory {
      ...
      private static LSHHeaderPage createHashFunctionHeaderPage(int startHeaderPageId, String indexName) throws ConstructPageException, HashEntryNotFoundException, FieldNumberOutOfBoundException, InvalidFrameNumberException, IOException, PageUnpinnedException, ReplacerException, InvalidTupleSizeException, InvalidTypeException {
      ...
      private static LSHHeaderPage createLayerHeaderPage(int h, int th, int startHeaderPageId, int layerId, String indexName) throws ConstructPageException, FieldNumberOutOfBoundException, IOException, HashEntryNotFoundException, InvalidFrameNumberException, PageUnpinnedException, ReplacerException, InvalidTupleSizeException, InvalidTypeException {
         ...
         LSHFInnerPage newInnerPage  = new LSHFInnerPage(layerId,0, indexName);
         layerPage.setLayerHeaderPage(startHeaderPageId, newInnerPage.getCurPage().pid, layerId,hashes);
         LSHLayer layer = new LSHLayer(hashes, newInnerPage.getCurPage().pid,layerId,indexName);
         LSHLayerMap.addLayer(layer, indexName);

         SystemDefs.JavabaseBM.unpinPage(newInnerPage.getCurPage(),true); // should it unpin here?
         return layerPage;

      }
      public static LSHHeaderPage createHeaderPages(int h, int L, String indexName) throws HashEntryNotFoundException, FieldNumberOutOfBoundException, ConstructPageException, InvalidFrameNumberException, IOException, PageUnpinnedException, ReplacerException, BufferPoolExceededException, PageNotReadException, HashOperationException, BufMgrException, PagePinnedException, InvalidTupleSizeException, InvalidTypeException {
         int th = h*2;
         int w = 100;
         ...
         for (int i =0;i<th;i++){
            LSHHeaderPage currentHeader =  createHashFunctionHeaderPage(baseHeaderPageId, indexName);
         ...
         for (int i=0;i<L;i++){
            LSHHeaderPage currentHeader = createLayerHeaderPage(h,th,baseHeaderPageId, i, indexName);
            prevHeaderPage.setNextPage(currentHeader.getCurPage());
            SystemDefs.JavabaseBM.unpinPage(prevHeaderPage.getCurPage(),true);
            prevHeaderPage = currentHeader;
         }
         ...
         LSHashFunctionsMap.getInstance(indexName);
         LSHLayerMap.getInstance(indexName);
