...
//       fldOffset = fromTuple.copyFldOffset();
    ...
    /**
     * Convert this field into a vector100D
     *
     * @param    fldNo   the field number
     * @return           the Vector100D if success
     *
     * @exception   IOException I/O errors
     * @exception   FieldNumberOutOfBoundException Tuple field number out of bound
     */
            ...
            throws IOException, FieldNumberOutOfBoundException
    {
        ...
        if ( (fldNo > 0) && (fldNo <= fldCnt))
        {
            val = Convert.getVector100DValue(fldOffset[fldNo -1], data);
            return val;
        }
        else
            throw new FieldNumberOutOfBoundException (null, "TUPLE:TUPLE_FLDNO_OUT_OF_BOUND");
            ...
            throws IOException, FieldNumberOutOfBoundException
    {
        if ( (fldNo > 0) && (fldNo <= fldCnt))
        {
            Convert.setVector100DValue(val, fldOffset[fldNo -1], data);
            return this;
        }
        else
            throw new FieldNumberOutOfBoundException (null, "TUPLE:TUPLE_FLDNO_OUT_OF_BOUND");
            ...
            throws IOException, InvalidTypeException, InvalidTupleSizeException
    {
        ...
        switch(types[numFlds -1].attrType) {
            ...
            case AttrType.attrVector100D:
                incr = 200; // 100 dimensions * 2 bytes
                break;
    ...
    public long compareTupleWithTuple(int fldNo1, Tuple t2, int fldNo2, AttrType[] type)
            throws IOException, FieldNumberOutOfBoundException {
        if ((fldNo1 <= 0 || fldNo1 > fldCnt) || (fldNo2 <= 0 || fldNo2 > t2.fldCnt)) {
            throw new FieldNumberOutOfBoundException(null, "TUPLE:TUPLE_FLDNO_OUT_OF_BOUND");
        }

        int thisOffset = fldOffset[fldNo1 - 1];
        int otherOffset = t2.fldOffset[fldNo2 - 1];
        AttrType fieldType = type[fldNo1 - 1];

        switch (fieldType.attrType) {
            case AttrType.attrVector100D:
                Vector100Dtype v1 = Convert.getVector100DValue(thisOffset, data);
                Vector100Dtype v2 = Convert.getVector100DValue(otherOffset, t2.data);
                return v1.distanceTo(v2); // Integer Euclidean distance
            case AttrType.attrInteger:
                int i1 = Convert.getIntValue(thisOffset, data);
                int i2 = Convert.getIntValue(otherOffset, t2.data);
                return Integer.compare(i1, i2);
            case AttrType.attrReal:
                float f1 = Convert.getFloValue(thisOffset, data);
                float f2 = Convert.getFloValue(otherOffset, t2.data);
                return Float.compare(f1, f2);
            case AttrType.attrString:
                String s1 = Convert.getStrValue(thisOffset, data, fldOffset[fldNo1] - thisOffset);
                String s2 = Convert.getStrValue(otherOffset, t2.data, t2.fldOffset[fldNo2] - otherOffset);
                return s1.compareTo(s2);
            default:
                throw new IOException("TUPLE: UNSUPPORTED_TYPE");
        }
    }

    public long compareTupleWithValue(int fldNo, Object value, AttrType type)
            throws IOException, FieldNumberOutOfBoundException {
        if (fldNo <= 0 || fldNo > fldCnt) {
            throw new FieldNumberOutOfBoundException(null, "TUPLE:TUPLE_FLDNO_OUT_OF_BOUND");
        }
        ...
        switch (type.attrType) {
            case AttrType.attrVector100D:
                Vector100Dtype v1 = Convert.getVector100DValue(offset, data);
                if (!(value instanceof Vector100Dtype)) {
                    throw new IOException("TUPLE: VALUE_TYPE_MISMATCH");
                }
                Vector100Dtype v2 = (Vector100Dtype) value;
                return v1.distanceTo(v2); // Integer Euclidean distance
            case AttrType.attrInteger:
                int i1 = Convert.getIntValue(offset, data);
                ...
                return Integer.compare(i1, i2);
            case AttrType.attrReal:
                float f1 = Convert.getFloValue(offset, data);
                ...
                return Float.compare(f1, f2);
            case AttrType.attrString:
                String s1 = Convert.getStrValue(offset, data, fldOffset[fldNo] - offset);
