...
public class TupleUtils {
    ...
    public static int CompareTupleWithTuple(AttrType fldType,
        ...
        switch (fldType.attrType) {
            ...
            case AttrType.attrVector100D:
                ...
                return (int) t1_v.getDistance(t2_v);
            default:
    ...
    public static int CompareTupleWithTuple(AttrType fldType,
                                            Tuple t1, int t1_fld_no,
                                            Tuple t2, int t2_fld_no, Vector100Dtype target){
        Vector100Dtype t1_v, t2_v;
        switch (fldType.attrType){
            case AttrType.attrVector100D :
                try {
                    t1_v = t1.get100DVectFld(t1_fld_no);
                    t2_v = t2.get100DVectFld(t2_fld_no);
                    long d1 = t1_v.distanceTo(target);
                    long d2 = t2_v.distanceTo(target);
                    if (d1<d2)return -1;
    ...
    public static int CompareTupleWithTuple(AttrType fldType,
                                            Tuple t1, int t1_fld_no,
                                            Tuple t2, int t2_fld_no, LSHLayer layer){
        Vector100Dtype t1_v, t2_v;
        switch (fldType.attrType){
            case AttrType.attrVector100D :
                try {
                    t1_v = t1.get100DVectFld(t1_fld_no);
                    t2_v = t2.get100DVectFld(t2_fld_no);
                    int[] ch1 = layer.getCompoundHash(t1_v);
                    int[] ch2 = layer.getCompoundHash(t2_v);
                    ...
                    // If hashes are the same, compare the actual vector values
//                    return Arrays.compare(t1_v.getVector(), t1_v.getVector());
//                    return 0;
                } catch (FieldNumberOutOfBoundException | IOException e) {
                    throw new RuntimeException(e);
                }
